...
public enum Config {
	...
	public static void init() {
		...
		if (!FILE.exists()) {
			...
			yml.set(STAT1_INS_VAL.name(), 100);
			yml.set(STAT2_INS_VAL.name(), 100);
			yml.set(STAT3_INS_VAL.name(), 100);
			yml.set(ITEM_MYSTERY_CHANCE.name(), 60.0);
			yml.set(ITEM_MYSTERY.name(), new Item(Material.BOOK).setName(Colors.fix("&a&lMYSTERY BOOK")).addLore(Colors.fix("&aJAKIES LORE")).addLore(Colors.fix("&bJAKIES LORE")).addEnchant(Enchantment.DURABILITY, 10).toIs());
			try {
				yml.save(FILE);
			} catch (IOException e) {
				e.printStackTrace();
